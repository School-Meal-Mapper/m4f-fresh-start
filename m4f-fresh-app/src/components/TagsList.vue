<template>
  <div class="tag-list">
    <tag-oval v-for="(tag, index) in includedTags" v-bind:key="index"
      :value="tag.value"
...
export default {
  ...
  computed: {
    includedTags: function () {
      const validTagNames = Object.keys(this.result.tags).filter((key) => { // find which tags are checked/have non empty lists
